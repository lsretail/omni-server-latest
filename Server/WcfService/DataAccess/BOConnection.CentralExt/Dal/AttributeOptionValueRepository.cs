...
namespace LSOmni.DataAccess.BOConnection.CentralExt.Dal
{
    public class AttributeOptionValueRepository : BaseRepository
    {
        ...
        public AttributeOptionValueRepository(BOConfiguration config, Version version) : base(config, version)
        {
        ...
        public List<ReplAttributeOptionValue> ReplicateEcommAttributeOptionValueTM(int batchSize, bool fullReplication, ref string lastKey, ref int recordsRemaining)
        {
            ...
            List<JscActions> actions = LoadDeleteActions(fullReplication, TABLEID, "LSC Attribute Option Value$5ecfc871-5d82-43f1-9c54-59685e82318d", keys, batchSize, ref delKey);
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = connection.CreateCommand())
                {
                    ...
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                        {
                            list.Add(ReaderToEcommAttributeOptionValue(reader, out mainKey));
                            cnt++;
                        }
                        reader.Close();
                        recordsRemaining -= cnt;
