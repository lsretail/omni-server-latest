...
namespace LSOmni.DataAccess.BOConnection.NavCommon.XmlMapping.Loyalty
{
    ...
    public class MenuXml : BaseXml
    {
        ...
        public string MenuGetAllRequestXML(string storeId, string hospType, string terminalId)
        {
            /*
            <?xml version="1.0"?>
            <Request>
            ...
            XElement root =
                new XElement("Request",
                    ...
                    new XElement("Request_Body",
                        ...
                        new XElement("Sales_Type", hospType),
                        new XElement("Terminal_No", terminalId)
                    )
                );
            ;
                ...
                if (mNode.Id == parentId)
                {
                    ...
                    else
                    {
                        if (entryType.ToLower().Contains("offer") == false)
                            logger.Info(string.Empty, "NOTE XML PARSE: No menunode item type found for id:{0} - {1}", nodeLine.Id, elMNodeLine.ToString());
                    }
            ...
            foreach (XElement elMNode in elMenuNodes)
            {
                ...
                else
                {
                    ...
                    if (elItem != null)
                    {
                        ...
                        {
                            if (entryType.ToLower().Contains("offer") == false)
                                logger.Info(string.Empty, "NOTE XML PARSE: No menunode item type found for id:{0} - {1}", mnLine.Id, elMNode.ToString());
                        }
                        //
                        mNode.MenuNodeLines.Add(mnLine);
                    }
