    ...
    public class TerminalRepository : BaseRepository
    {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = connection.CreateCommand())
                {
                    command.CommandText = "SELECT mlr.[Device License Key]" +
                                         " FROM [" + navCompanyName + "MobileLicenseRegistration$5ecfc871-5d82-43f1-9c54-59685e82318d] mlr" +
        ...
        private void GetFeatureFlags(FeatureFlags flags, string storeId, string terminalId)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = connection.CreateCommand())
                {
                    command.CommandText = "SELECT mt.[Feature Flag],mt.[Value] FROM [" + navCompanyName + "Feature Flags Setup$5ecfc871-5d82-43f1-9c54-59685e82318d] mt " +
