...
namespace LSOmni.DataAccess.BOConnection.CentrAL.Dal
{
    public class ExtendedVariantValuesRepository : BaseRepository
    {
        ...
        public ExtendedVariantValuesRepository(BOConfiguration config, Version navVersion) : base(config, navVersion)
        {
            ...
            sqlfrom = " FROM [" + navCompanyName + "Extended Variant Values$5ecfc871-5d82-43f1-9c54-59685e82318d] mt";
        }
        ...
        public List<ReplExtendedVariantValue> ReplicateExtendedVariantValues(string storeId, int batchSize, bool fullReplication, ref string lastKey, ref string maxKey, ref int recordsRemaining)
        {
            ...
            List<JscKey> keys = GetPrimaryKeys("Extended Variant Values$5ecfc871-5d82-43f1-9c54-59685e82318d");
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = connection.CreateCommand())
                {
                    ...
                    command.CommandText = "SELECT 1 FROM sys.columns WHERE [Name]='Logical Order' AND Object_ID=Object_ID('[" + navCompanyName + "Extended Variant Dimensions$5ecfc871-5d82-43f1-9c54-59685e82318d]')";
                    bool hascol = SQLHelper.GetBool(command.ExecuteScalar());

                    if (hascol)
                    {
                        sqlcolumns += ",(SELECT vd.[Logical Order] FROM [" + navCompanyName + "Extended Variant Dimensions$5ecfc871-5d82-43f1-9c54-59685e82318d] vd " +
        ...
        public List<VariantExt> VariantRegGetByItemId(string itemId)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = connection.CreateCommand())
                {
                    ...
                    if (hascol)
                    {
                        command.CommandText += ",(SELECT vd.[Logical Order] FROM [" + navCompanyName + "Extended Variant Dimensions$5ecfc871-5d82-43f1-9c54-59685e82318d] vd " +
                                      "WHERE vd.[Framework Code]=mt.[Framework Code] AND vd.[Dimension No_]=mt.[Dimension] AND vd.[Item]='') AS DOrder";
                    }
