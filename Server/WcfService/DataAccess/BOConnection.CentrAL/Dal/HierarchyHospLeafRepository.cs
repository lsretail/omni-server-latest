    ...
    public class HierarchyHospLeafRepository : BaseRepository
    {
        ...
        public HierarchyHospLeafRepository(BOConfiguration config, Version navVersion) : base(config, navVersion)
        {
            ...
            sqlcolumnsModifier = "nl.[Hierarchy Code],nl.[Node ID],nl.[No_],mt.[Code],mt.[Subcode],mt.[Description],mt.[Trigger Code],mt.[Price Type]," +
                                 "mt.[Price Handling],mt.[Amount _Percent],mt.[Unit of Measure],mt.[Max_ Selection],mt.[Min_ Selection],tsi.[Usage Sub-Category]";
            sqlfromModifier = " FROM [" + navCompanyName + "Hierarchy Node Link$5ecfc871-5d82-43f1-9c54-59685e82318d] nl" +
                              " INNER JOIN [" + navCompanyName + "Hierarchy Date$5ecfc871-5d82-43f1-9c54-59685e82318d] hd ON hd.[Hierarchy Code]=nl.[Hierarchy Code] AND hd.[Start Date]<=GETDATE()" +
                              " LEFT OUTER JOIN [" + navCompanyName + "Table Specific Infocode$5ecfc871-5d82-43f1-9c54-59685e82318d] tsi ON tsi.[Value]=nl.[No_] AND tsi.[Usage Category]=2" +
        ...
        private ReplHierarchyHospModifier ReaderToHierarchyModifier(SqlDataReader reader, out string timestamp)
        {
            ...
            return new ReplHierarchyHospModifier()
            {
                ...
                Type = (ModifierType)SQLHelper.GetInt32(reader["Usage Sub-Category"]),
                UnitOfMeasure = SQLHelper.GetString(reader["Unit of Measure"]),
