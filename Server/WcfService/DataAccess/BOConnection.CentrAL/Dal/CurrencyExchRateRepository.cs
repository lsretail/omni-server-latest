...
namespace LSOmni.DataAccess.BOConnection.CentrAL.Dal
{
    public class CurrencyExchRateRepository : BaseRepository
    {
        ...
        {
            sqlcolumns = "mt.[Currency Code],mt.[Starting Date],mt2.[POS Exchange Rate Amount],mt2.[POS Rel_ Exch_ Rate Amount]," +
        ...
        private ReplCurrencyExchRate ReaderToCurrencyExchRate(SqlDataReader reader, out string timestamp, bool getTS)
        {
            ...
            {
                currency.CurrencyFactor = ((1 / SQLHelper.GetDecimal(reader, "POS Exchange Rate Amount")) * SQLHelper.GetDecimal(reader, "POS Rel_ Exch_ Rate Amount"));
            }
            else
            {
