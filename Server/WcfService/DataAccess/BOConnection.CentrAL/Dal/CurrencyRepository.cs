    ...
    public class CurrencyRepository : BaseRepository
    {
        ...
        public CurrencyRepository(BOConfiguration config, Version navVersion) : base(config, navVersion)
        {
            sqlcolumns = "mt.[Code],mt.[Description],mt.[Amount Rounding Precision],mt.[Invoice Rounding Type]," +
                         "mt.[Invoice Rounding Precision],mt2.[POS Currency Symbol],mt2.[Placement Of Currency Symbol]";
        ...
        private ReplCurrency ReaderToCurrency(SqlDataReader reader, out string timestamp)
        {
            ...
            int cursymplacement = SQLHelper.GetInt32(reader["Placement Of Currency Symbol"]);
