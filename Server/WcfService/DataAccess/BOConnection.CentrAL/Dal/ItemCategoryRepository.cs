    ...
    public class ItemCategoryRepository : BaseRepository
    {
        ...
        public ItemCategoryRepository(BOConfiguration config, Version navVersion) : base(config, navVersion)
        {
            ...
            sqlfrom = " FROM [" + navCompanyName + "Item Category$437dbf0e-84ff-417a-965d-ed2bb9650972] mt" +
                      " LEFT OUTER JOIN [" + navCompanyName + "Product Group$437dbf0e-84ff-417a-965d-ed2bb9650972] pg ON pg.[Item Category Code]=mt.[Code]" +
                      " LEFT OUTER JOIN [" + navCompanyName + "Item$437dbf0e-84ff-417a-965d-ed2bb9650972] it ON it.[Product Group Code]=pg.[Code]";
        }
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = connection.CreateCommand())
                {
                    command.CommandText = "SELECT DISTINCT " + sqlcolumns + sqlfrom + 
                            " WHERE EXISTS(" +
                            " SELECT 1 FROM [" + navCompanyName + "Product Group$437dbf0e-84ff-417a-965d-ed2bb9650972] pg " +
                            " INNER JOIN [" + navCompanyName + "Item$437dbf0e-84ff-417a-965d-ed2bb9650972] i ON i.[Product Group Code]=pg.[Code] " +
