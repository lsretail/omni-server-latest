...
namespace LSOmni.DataAccess.BOConnection.NavSQL.Dal
{
    public class UnitOfMeasureRepository : BaseRepository
    {
        ...
        public List<ReplCollection> ReplicateCollection(string storeId, int batchSize, bool fullReplication, ref string lastKey, ref string maxKey, ref int recordsRemaining)
        {
            ...
            List<JscKey> keys = GetPrimaryKeys("Collection Framework");

            // get records remaining
            string sql = string.Empty;
            if (fullReplication)
            {
                sql = "SELECT COUNT(*) FROM [" + navCompanyName + "Collection Framework] mt" + GetWhereStatementWithStoreDist(true, keys, "mt.[Item]", storeId, false);
            }
            ...
            // get records
            sql = GetSQL(fullReplication, batchSize) + "mt.[Unit Of Measure],mt.[Item],mt.[Variant],mt.[Qty_] FROM [" + navCompanyName + "Collection Framework] mt" + GetWhereStatementWithStoreDist(true, keys, "mt.[Item]", storeId, false);
