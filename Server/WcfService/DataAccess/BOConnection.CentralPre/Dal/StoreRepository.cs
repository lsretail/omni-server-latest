        ...
        public Store StoreLoyGetById(string storeId, bool includeDetails)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = connection.CreateCommand())
                {
                    ...
                    command.CommandText = "SELECT " + sqlcolumns + " FROM [" + navCompanyName + "LSC Store$5ecfc871-5d82-43f1-9c54-59685e82318d] mt WHERE mt.[No_]=@id";
                    command.Parameters.AddWithValue("@id", storeId);
                    TraceSqlCommand(command);
                    connection.Open();
                    using (SqlDataReader reader = command.ExecuteReader())
        ...
        public List<Store> StoreLoyGetAll(bool clickAndCollectOnly)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = connection.CreateCommand())
                {
                    ...
                    command.CommandText = "SELECT " + sqlcolumns + " FROM [" + navCompanyName + "LSC Store$5ecfc871-5d82-43f1-9c54-59685e82318d] mt " + where + " ORDER BY mt.[Name]";
                    TraceSqlCommand(command);
                    connection.Open();
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
