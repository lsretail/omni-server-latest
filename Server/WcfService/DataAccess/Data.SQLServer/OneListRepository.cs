    ...
    public class OneListRepository : BaseRepository, IOneListRepository
    {
            ...
            : base(config)
        {
            sqlcol = "mt.[Id],mt.[ExternalType],mt.[Description],mt.[StoreId],mt.[ListType],mt.[SalesType],mt.[IsHospitality]," +
                     ...
                     "mt.[ShippingAmount],mt.[CreateDate],mt.[Currency] ";
                        ...
                        using (SqlCommand command = connection.CreateCommand())
                        {
                            lock (lockOneList)
                            {
                                ...
                                command.CommandText = "IF EXISTS(SELECT * FROM [OneList] WHERE [Id]=@id) " +
                                                      ...
                                                      "[TotalNetAmount]=@f7,[TotalTaxAmount]=@f8,[TotalDiscAmount]=@f9,[ShippingAmount]=@f10,[PointAmount]=@f11,[LastAccessed]=@f13,[StoreId]=@f14,[Currency]=@f15 " +
                                                      "WHERE [Id]=@id" +
                                                      " ELSE " +
                                                      "INSERT INTO [OneList] (" +
                                                      "[Id],[ExternalType],[Description],[ListType],[SalesType],[IsHospitality],[TotalAmount]," +
                                                      "[TotalNetAmount],[TotalTaxAmount],[TotalDiscAmount],[ShippingAmount],[PointAmount],[CreateDate],[LastAccessed],[StoreId],[Currency]" +
                                                      ") VALUES (@id,@f1,@f2,@type,@f3,@f5,@f6,@f7,@f8,@f9,@f10,@f11,@f12,@f13,@f14,@f15)";
                                ...
                                command.Parameters.AddWithValue("@f15", NullToString(list.Currency, 5));
                                TraceSqlCommand(command);
            ...
            using (SqlConnection connection = new SqlConnection(sqlConnectionString))
            {
                using (SqlCommand command = connection.CreateCommand())
                {
                    command.CommandText = "SELECT [Id],[OneListId],[DisplayOrderId],[ItemId],[UomId],[VariantId]," +
                        "[ItemDescription],[VariantDescription],[ItemCategory],[ProductGroup],[ImageId],[BarcodeId],[Location],[IsADeal],[IsManual]," +
                        "[Quantity],[NetPrice],[Price],[NetAmount],[TaxAmount],[DiscountAmount],[DiscountPercent],[CreateDate],[Immutable]" +
                    ...
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            OneListItem line = new OneListItem()
                            {
                                ...
                                IsManualItem = SQLHelper.GetBool(reader["IsManual"]),
                                Immutable = SQLHelper.GetBool(reader["Immutable"])
                            };
        ...
        private void OneListItemSave(string oneListId, ObservableCollection<OneListItem> listLines, SqlConnection db, SqlTransaction trans, bool calculate)
        {
            using (SqlCommand command = db.CreateCommand())
            {
                ...
                command.CommandText = "INSERT INTO [OneListItem] (" +
                    ...
                    "[Quantity],[NetAmount],[TaxAmount],[Price],[NetPrice],[DiscountAmount],[DiscountPercent],[CreateDate],[Location],[IsADeal],[ItemCategory],[ProductGroup],[IsManual],[Immutable]" +
                    ") VALUES (@f0,@f1,@f2,@f3,@f4,@f5,@f6,@f7,@f8,@f9,@f10,@f11,@f12,@f13,@f14,@f15,@f16,@f17,@f18,@f19,@f20,@f21,@f22,@f23)";
                ...
                command.Parameters.Add("@f22", SqlDbType.TinyInt);
                command.Parameters.Add("@f23", SqlDbType.TinyInt);
                ...
                foreach (OneListItem line in listLines)
                {
                    ...
                    command.Parameters["@f22"].Value = line.IsManualItem;
                    command.Parameters["@f23"].Value = line.Immutable;
                    TraceSqlCommand(command);
                    command.ExecuteNonQuery();
                    ...
                    if (calculate && (line.OnelistItemDiscounts != null && line.OnelistItemDiscounts.Count > 0))
                    {
                        using (SqlCommand command2 = db.CreateCommand())
                        {
                            ...
                            {
                                command2.Parameters["@f0"].Value = GuidHelper.NewGuidString();
                    ...
                    if (line.OnelistSubLines != null && line.OnelistSubLines.Count > 0)
                    {
                        using (SqlCommand command3 = db.CreateCommand())
                        {
                            ...
                            {
                                command3.Parameters["@f0"].Value = GuidHelper.NewGuidString();
        ...
        private OneList ReaderToOneList(SqlDataReader reader, bool includeLines)
        {
            OneList oneList = new OneList()
            {
                ...
                Currency = SQLHelper.GetString(reader["Currency"]),
