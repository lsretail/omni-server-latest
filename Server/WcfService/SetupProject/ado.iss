...
begin
    ...
    // if (VarIsEmpty(ADOGlobalConnection))  then
     begin
     ...
     //if (VarIsEmpty(ADOGlobalCommand))  then
     begin
     ...
     //never do ADOGlobalConnection.Close;
end;
  ...
  try
    if ADOLoadScriptFromFile(FileName, CommandList) > 0 then
      ...
      begin
        ...
        //Log('ADORunScript: I= ' + IntToStr(I) );   
        ADOExecuteSQL(CommandList[I]);
      end;
      //ADOGlobalConnection.Close;
  except
      //reset so next adoconnection string is correct
